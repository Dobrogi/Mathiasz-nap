...
{ 
    ...
    if (notallowed_number.length!=7){
        ...
        if (sum == 7)
        {
            result.innerHTML = sum + " / 7 pont – 🧠 Tökéletes! Mester vagy a gazdaság és menedzsment témában!"
        }

        else if (sum == 6)
        {
            result.innerHTML = sum + " / 7 pont – 💼 Nagyon jó! Csak egy hajszál választ el a tökéletestől."
        }

         else if (sum == 5)
        {
            result.innerHTML = sum + " / 7 pont – 📈 Szép munka! A tudásod stabil és jól használható."
        }

         else if (sum == 4)
        {
            result.innerHTML = sum + " / 7 pont – 📘 Nem rossz! Az alapok mennek, de van még mit csiszolni."
        }

        else if (sum == 3)
        {
            result.innerHTML = sum + " / 7 pont – 🔍 Fejlődő szint. Jó kezdés, érdemes tovább gyakorolni."
        }

        else if (sum == 1 || sum == 2)
        {
            result.innerHTML = sum + " / 7 pont – 🪄 Kezdő szint. Ne csüggedj, a gyakorlás meghozza a sikert!"
        }

        else
        {
            result.innerHTML = sum + " / 7 pont – 💤 Hoppá! Itt az idő egy kis tanulásra – újra neki lehet futni!"
        }
    }
